...
type Notification = {
  ...
  // Propriétés pour les notifications groupées
  isGrouped?: boolean;
  count?: number;
  firstUsers?: string[];
  reason?: string;
...
const TYPE_LABEL: Record<string, string> = {
  ...
  BOOKING_RESCHEDULE: "a proposé un nouveau créneau",
  BOOKING_CANCELLED:  "a annulé la réservation",
  BOOKING_REMINDER:   "Votre appel démarre dans 15 minutes",
  BOOKING_START:      "Votre appel a commencé",
  IDENTITY_VERIFIED:  "Votre demande d'identification a été validée",
  IDENTITY_REJECTED:  "Votre demande d'identification a été refusée",
};
...
export default function NotificationsPanel({ onClose }: { onClose: () => void }) {
  ...
  // Fonction pour générer le texte des notifications groupées
  const getGroupText = (notif: Notification) => {
    const users = notif.firstUsers?.join(", ") || "";
    const othersCount = (notif.count || 0) - (notif.firstUsers?.length || 0);
    
    if (notif.type === "LIKE") {
      if (othersCount > 0) {
        return `${users} et ${othersCount} ${othersCount > 1 ? 'autres personnes' : 'autre personne'} ont aimé votre publication`;
      }
      return `${users} a aimé votre publication`;
    }
    
    if (notif.type === "COMMENT") {
      if (othersCount > 0) {
        return `${users} et ${othersCount} ${othersCount > 1 ? 'autres personnes' : 'autre personne'} ont commenté votre publication`;
      }
      return `${users} a commenté votre publication`;
    }
