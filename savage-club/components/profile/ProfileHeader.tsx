              ...
              @{isOwner ? (
                <InlineField
                  ...
                  onSave={async (v) => {
                    ...
                    if (!regex.test(v)) {
                      setSaveError("3–30 caractères, lettres/chiffres/underscore uniquement");
                      return;
                    }
                    const res = await fetch(`/api/check-username?username=${encodeURIComponent(v)}`);
                    const data = await res.json();
                    if (!data.available) {
                      setSaveError("Ce nom d'utilisateur est déjà pris");
                      return;
                    }
                    await saveField("username", v);
                  }}
                  placeholder="pseudo"
