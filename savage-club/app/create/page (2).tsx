...
export default function CreatePage() {
  ...
  // Seuls CREATOR et TRAINER peuvent choisir la visibilité
  ...
  async function uploadFile(file: File, index: number) {
    setMedias((prev) =>
      prev.map((m, i) => i === index ? { ...m, uploading: true, error: null } : m)
    );
    try {
      ...
      setMedias((prev) =>
        prev.map((m, i) => i === index ? { ...m, uploadedUrl: data.url, uploading: false } : m)
      );
    } catch (err: any) {
      setMedias((prev) =>
        prev.map((m, i) => i === index ? { ...m, error: err.message, uploading: false } : m)
      );
    }
  }
  ...
  const canSubmit = (medias.length > 0 || content.trim().length > 0) && !submitting && (medias.length === 0 || allUploaded);
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-xl mx-auto px-4 py-8">
              ...
              <div className="mb-2">
                ...
                <div className={`relative overflow-hidden bg-black rounded-xl ${
                  ...
                  <div className="absolute top-3 left-3 flex gap-1.5">
                    ...
                    {medias[activeIndex]?.uploadedUrl && (
                      <span className="bg-green-500/80 text-white text-xs px-2 py-1 rounded-full">✓ Prêt</span>
                    )}
                    {medias[activeIndex]?.error && (
                      <span className="bg-red-500/80 text-white text-xs px-2 py-1 rounded-full">⚠ Erreur</span>
                    )}
                  </div>

                  <button
                    onClick={() => removeMedia(activeIndex)}
                    className="absolute bottom-3 right-3 bg-black/60 hover:bg-red-500/80 text-white w-8 h-8 rounded-full flex items-center justify-center transition-colors"
                  >
                    ...
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1">
                      {medias.map((_, i) => (
                        <button key={i} onClick={() => setActiveIndex(i)}
                          className={`h-1.5 rounded-full transition-all ${i === activeIndex ? "bg-white w-3" : "bg-white/40 w-1.5"}`}
                        />
                      ))}
                    </div>
                  )}
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                {medias.map((m, i) => (
                  <button key={i} onClick={() => setActiveIndex(i)}
                    className={`relative flex-shrink-0 w-14 h-14 rounded-xl overflow-hidden border-2 transition-all ${
                      i === activeIndex ? "border-amber-400" : "border-transparent opacity-50 hover:opacity-80"
                    }`}
                  >
                    {m.type === "VIDEO"
                      ? <video src={m.localUrl} className="w-full h-full object-cover" muted playsInline/>
                ...
                {medias.length < MAX_FILES && (
                  <button onClick={() => fileInputRef.current?.click()}
                    className="flex-shrink-0 w-14 h-14 rounded-xl border-2 border-dashed border-white/20 hover:border-amber-400/50 flex items-center justify-center transition-colors"
                  >
          ...
          <div className="space-y-3 mt-4">
            ...
            {/* Visibilité — CREATOR et TRAINER uniquement */}
            {canSetVisibility && (
              <>
